...
# ── Reconstruct table rows from Y positions ───────────────────
...
def extract_customer_name_text(text):
    """
    Name is split across two OCR lines:
    Line 1: اسم العميل :شركة لألأة النجو  قم الغاتورة 02975
    Line 2: تاريخ الغاتورة ... إلىة التتجار
    Grab Arabic from line 1 before invoice number +
    Arabic from next line before تاريخ
    """
    part1_words = []
    m1 = re.search(
...
# ── Item extraction: positional (OCR) ─────────────────────────
...
# ── Item extraction: native (pdfplumber) ──────────────────────
