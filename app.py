...
# Pack-size tokens that pollute SKU: "18 ك", "(510)", "18 كجم", etc.
...
def parse_item_line(line):
    ...
    Fix 1 – Description:
        Collect ALL English words from the FULL line so tokens that appear
        before, between, or after the Arabic block (e.g. 'VEAL' OCR-misread
        as 'ادعلا' on some pages but correct on 2975) are never dropped.
        Keeps words ≥ 3 chars OR fully uppercase ≥ 2 chars.

    Fix 2 – SKU:
        Apply clean_sku() to strip pack-size tokens (18 ك, (510), …) and
        stray digits so they don't pollute the Arabic product name.
    """
    ...
    candidates = data_nums[:-1]   # drop last number (= total)
    ...
    # fallback when no decimal found
    ...
    # ── Fix 1: description from ALL English words in the full line ──
...
def extract_metadata(pdf_path, text):
    ...
    # Fix 3 — Paid fallback: OCR often garbles مدفوع line into noise.
    # If الرصيد المستحق = 0, the invoice is fully paid → Paid = total_after.
    if not paid:
