...
def extract_metadata(pdf_path):
    try:
        ...
        if len(full_text.strip()) < 50:
            st.warning(f"⚠️ Using OCR for {pdf_path.name}")
                ...
                for table in tables:
                    if table:
                        ...
                        if merged_rows:
                            ...
                            headers = ["Total before tax", "الكمية", "Unit price", "Quantity", "Description", "SKU", "Extra"]
                            df_cleaned = pd.DataFrame(merged_rows, columns=headers[:num_cols])
                            all_data.append(df_cleaned)

            # 🔥 OCR fallback if no tables
            if not found_table:
                st.warning(f"⚠️ No tables found, using OCR for {pdf_path.name}")
...
st.title("📄 Invoice Extractor (PDF → Excel)")
